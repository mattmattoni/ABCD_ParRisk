...
int_ptab <- data.frame(matrix(NA, nrow = 28, ncol = 7))     # Adjust nrow in ptab if number of iVar is changed
names(int_m) <- dVar
names(int_d) <- dVar
...
#for loop
    for (i in 1:no.dVar) {
        ...
        int_ptab[[i]]$int_d[2:28] <- int_d[[i]]$d
        int_ptab[[i]]$int_d[2:28] = round(int_ptab[[i]]$int_d[2:28], 3)  #3 decimals
...
#Interaction
scgv_int <- lmer(win_smri_vol_scs_subcorticalgv.z ~ age.z + sexF + raceF + puberty.z + eduF + income.z + Dep*Anx*Man*SU + (1|siteF/famF), data=data, na.action = "na.omit")
...
rownames(pVals)<- c('Dep_Ind','Anx_Ind','Man_Ind','SU_Ind','Dep_Main','Anx_Main','Man_Main','SU_Main',
                    'Dep_Int','Anx_Int','Man_Int','SU_Int','Dep*Anx','Dep*Man','Anx*Man','Dep*SU','Anx*SU','Man*SU')
...
#No interaction terms are significant, these models will no longer be considered
#Remove interaction items
pVals <- pVals[1:8,]
#BH Correction
